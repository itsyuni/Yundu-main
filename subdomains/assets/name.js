 ...
 *
 * let rn = new RussianName('Паниковский Михаил Самуэльевич');
...
let RussianNameProcessor = {
	...
	applyMod: function(word, gcase, rule) {
		switch(gcase) {
			case this.gcaseNom: let mod = '.'; break;
			case this.gcaseGen: let mod = rule.mods[0]; break;
			case this.gcaseDat: let mod = rule.mods[1]; break;
			case this.gcaseAcc: let mod = rule.mods[2]; break;
			case this.gcaseIns: let mod = rule.mods[3]; break;
			case this.gcasePos: let mod = rule.mods[4]; break;
...
let RussianName = function(lastName, firstName, middleName, sex) {
	if(typeof firstName == 'undefined') {
		let m = lastName.match(/^\s*(\S+)(\s+(\S+)(\s+(\S+))?)?\s*$/);
